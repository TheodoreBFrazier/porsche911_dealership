\c data_dev;

INSERT INTO
    carrera_models(
        model_name,
        price,
        power_in_hp,
        top_speed_mph,
        zero_to_sixty,
        description,
        height,
        width,
        length,
        curb_weight,
        wheelbase,
        motor,
        bore,
        stroke,
        displacement,
        max_power,
        max_power_at_rpm,
        max_engine_speed,
        max_torque,
        max_torque_at_rpm,
        max_power_per_liter,
        power_to_weight,
        image
    )
VALUES
    (
        'Carrera Coupe',
        106100,
        379,
        182,
        4.0,
        'A icon reborn. With instantly recognizable styling and the timeless flat-six, now turbo charged. When it comes to pratical sports cars, the 911 is still very difficult to beat.',
        '51.1 in',
        '72.9 in',
        '177.9 in',
        '3,354 lb',
        '96.5 in',
        '3.0-liter twin turbocharged boxer 6',
        '91.0 mm',
        '76.4 mm',
        '2.981 cc',
        '379 hp',
        '6,500 rpm',
        '7,500 rpm',
        '331 lb-ft',
        '1,950 - 5,000 rpm',
        '126.3 hp/l',
        '8.8 lb/hp',
        'carrera.png'
    ),
    (
        'Carrera Cabriolet',
        118900,
        379,
        180,
        4.2,
        'The open-top variant of the 911 heightens the 911 experience by adding freedom and fresh air. When closed, the Cabriolet maintains the same clean lines of the coupe models.',
        '51.1 in',
        '72.9 in',
        '177.9 in',
        '3,354 lb',
        '96.5 in',
        '3.0-liter twin turbocharged boxer 6',
        '91.0 mm',
        '76.4 mm',
        '2.981 cc',
        '379 hp',
        '6,500 rpm',
        '7,500 rpm',
        '331 lb-ft',
        '1,950 - 5,000 rpm',
        '126.3 hp/l',
        '9.3 lb/hp',
        'carrera-cabriolet.png'
    ),
    (
        'Carrera 4 Coupe',
        113400,
        379,
        180,
        4.0,
        'The 4 in Porsche Designation represents all wheel drive. The Carrera 4 adds additional traction over the standard Carrera.',
        '51.1 in',
        '72.9 in',
        '177.9 in',
        '3,354 lb',
        '96.5 in',
        '3.0-liter twin turbocharged boxer 6',
        '91.0 mm',
        '76.4 mm',
        '2.981 cc',
        '379 hp',
        '6,500 rpm',
        '7,500 rpm',
        '331 lb-ft',
        '1,950 - 5,000 rpm',
        '126.3 hp/l',
        '9.1 lb/hp',
        'carrera-4.png'
    ),
    (
        'Careera 4 Cabriolet',
        126200,
        179,
        379,
        4.2,
        'With all-while drive and a state of the art folding top, the Carrera 4 Cabriolet is entry into the perfect year around sports car.',
        '51.1 in',
        '72.9 in',
        '177.9 in',
        '3,354 lb',
        '96.5 in',
        '3.0-liter twin turbocharged boxer 6',
        '91.0 mm',
        '76.4 mm',
        '2.981 cc',
        '379 hp',
        '6,500 rpm',
        '7,500 rpm',
        '331 lb-ft',
        '1,950 - 5,000 rpm',
        '126.3 hp/l',
        '9.5 lb/hp',
        'carrera-4-cabriolet.png'
    ),
    (
        'Carrera S Coupe',
        123000,
        191,
        443,
        3.5,
        'S stands for Sport an enhanced performance. The Carrera S provides enhanced power output over the the non-sport models.',
        '51.1 in',
        '72.9 in',
        '177.9 in',
        '3,354 lb',
        '96.5 in',
        '3.0-liter twin turbocharged boxer 6',
        '91.0 mm',
        '76.4 mm',
        '2.981 cc',
        '443 hp',
        '6,500 rpm',
        '7,500 rpm',
        '390 lb-ft',
        '2,300 - 5,000 rpm',
        '147.6 hp/l',
        '7.6 lb/hp',
        'carrera-s.png'
    ),
    (
        'Carrera S Cabriolet',
        135800,
        190,
        443,
        3.7,
        'The additional performance of S, with extra sunlight and air.',
        '51.1 in',
        '72.9 in',
        '177.9 in',
        '3,354 lb',
        '96.5 in',
        '3.0-liter twin turbocharged boxer 6',
        '91.0 mm',
        '76.4 mm',
        '2.981 cc',
        '443 hp',
        '6,500 rpm',
        '7,500 rpm',
        '390 lb-ft',
        '2,300 - 5,000 rpm',
        '147.6 hp/l',
        '8.0 lb/hp',
        'carrera-s-cabriolet.png'
    ),
    (
        'Carrera 4S Coupe',
        130300,
        190,
        443,
        3.4,
        'Not sure to go all-wheel drive or enhanced perfmance? Have both with the Carrera 4S',
        '51.1 in',
        '72.9 in',
        '177.9 in',
        '3,658 lb',
        '96.5 in',
        '3.0-liter twin turbocharged boxer 6',
        '91.0 mm',
        '76.4 mm',
        '2.981 cc',
        '443 hp',
        '6,500 rpm',
        '7,500 rpm',
        '390 lb-ft',
        '2,300 - 5,000 rpm',
        '147.6 hp/l',
        '7.9 lb/hp',
        'carrera-4s-coupe.png'
    ),
    (
        'Carrera 4S Cabriolet',
        143100,
        188,
        443,
        3.6,
        'Extra speed. Extra grip. Extra sun. Extra Air. Enough said.',
        '51.1 in',
        '72.9 in',
        '177.9 in',
        '3,658 lb',
        '96.5 in',
        '3.0-liter twin turbocharged boxer 6',
        '91.0 mm',
        '76.4 mm',
        '2.981 cc',
        '443 hp',
        '6,500 rpm',
        '7,500 rpm',
        '390 lb-ft',
        '2,300 - 5,000 rpm',
        '147.6 hp/l',
        '8.2 lb/hp',
        'carrera-4s-cab.png'
    ),
    (
        'Targa 4 ',
        126200,
        379,
        179,
        4.2,
        'Arguably the most stylish 911 model. The Targa incorperates the characteristic rollbar with an innovative roof concept. A future classic that combines the experience of an open tpp with the comfort and safety of an enclsoed car.',
        '51.1 in',
        '72.9 in',
        '177.9 in',
        '3,658 lb',
        '96.5 in',
        '3.0-liter twin turbocharged boxer 6',
        '91.0 mm',
        '76.4 mm',
        '2.981 cc',
        '379 hp',
        '6,500 rpm',
        '7,500 rpm',
        '331 lb-ft',
        '2,300 - 5,000 rpm',
        '126.3 hp/l',
        '9.7 lb/hp',
        'targa-4.png'
    ),
    (
        'Targa 4S Targa',
        143100,
        443,
        188,
        3.6,
        'The style and experience of the iconic Porsche Targa, with the added traction and security of four wheel drive.',
        '50.9 in',
        '72.9 in',
        '178.5 in',
        '3,658 lb',
        '96.5 in',
        '3.0-liter twin turbocharged boxer 6',
        '91.0 mm',
        '76.4 mm',
        '2.981 cc',
        '443 hp',
        '6,500 rpm',
        '7,500 rpm',
        '390 lb-ft',
        '2,300 - 5,000 rpm',
        '126.3 hp/l',
        '8.3 lb/hp',
        'targa-4s.png'
    ),
    (
        'GTS Coupe',
        142600,
        473,
        193,
        3.2,
        'GTS models have not only enhanced perfomance, but a much sportier specification. GTS coupe is the most driver focused of standard Carreras.',
        '3.0-liter twin turbocharged boxer 6',
        '50.9 in',
        '72.9 in',
        '178.5 in',
        '3,658 lb',
        '96.5 in',
        '91.0 mm',
        '76.4 mm',
        '2.981 cc',
        '473 hp',
        '6,500 rpm',
        '7,500 rpm',
        '420 lb-ft',
        '2,300 - 5,000 rpm',
        '158.7 hp/l',
        '7.3 lb/hp',
        'gts.png'
    ),
    (
        'GTS Cabriolet',
        155400,
        473,
        192,
        3.4,
        'Lightweight GTS package with open top make the GTS cabriolet the posterchild for canyon carving',
        '50.8 in',
        '72.9 in',
        '178.5 in',
        '3,501 lb',
        '96.5 in',
        '3.0-liter twin turbocharged boxer 6',
        '91.0 mm',
        '76.4 mm',
        '2.981 cc',
        '473 hp',
        '6,500 rpm',
        '7,500 rpm',
        '420 lb-ft',
        '2,300 - 5,000 rpm',
        '158.7 hp/l',
        '7.5 lb/hp',
        'gts-cab.png'
    ),
    (
        'Carrera 4 GTS Coupe',
        149900,
        192,
        473,
        3.1,
        '4 GTS designation combines the sporting package with all wheel drive.',
        '50.9 in',
        '72.9 in',
        '178.5 in',
        '3,452 lb',
        '96.5 in',
        '3.0-liter twin turbocharged boxer 6',
        '91.0 mm',
        '76.4 mm',
        '2.981 cc',
        '473 hp',
        '6,500 rpm',
        '7,500 rpm',
        '420 lb-ft',
        '2,300 - 5,000 rpm',
        '158.7 hp/l',
        '7.5 lb/hp',
        'gts-4.png'
    ),
    (
        'Carrera 4 GTS Cabriolet',
        162700,
        190,
        473,
        3.3,
        'All wheel drive for winter and an insulated convertible top for summer. 4 GTS cabriolet is the perfect year-around companion.',
        '50.8 in',
        '72.9 in',
        '178.5 in',
        '3,605 lb',
        '96.5 in',
        '3.0-liter twin turbocharged boxer 6',
        '91.0 mm',
        '76.4 mm',
        '2.981 cc',
        '473 hp',
        '6,500 rpm',
        '7,500 rpm',
        '420 lb-ft',
        '2,300 - 5,000 rpm',
        '158.7 hp/l',
        '7.8 lb/hp',
        'carrera-4-gts-cab.png'
    ),
    (
        'Targa 4 GTS Targa',
        162700,
        473,
        190,
        3.3,
        'High style of Targa. High practicality of 4. High performance of GTS.',
        '51.1 in',
        '72.9 in',
        '178.5 in',
        '3,653 lb',
        '96.5 in',
        '3.0-liter twin turbocharged boxer 6',
        '91.0 mm',
        '76.4 mm',
        '2.981 cc',
        '473 hp',
        '6,500 rpm',
        '7,500 rpm',
        '420 lb-ft',
        '2,300 - 5,000 rpm',
        '158.7 hp/l',
        '7.9 lb/hp',
        '4-gts-targa.png'
    );

INSERT INTO
    turbo_models(
        model_name,
        price,
        power_in_hp,
        top_speed_mph,
        zero_to_sixty,
        description,
        height,
        width,
        length,
        curb_weight,
        wheelbase,
        motor,
        bore,
        stroke,
        displacement,
        max_power,
        max_power_at_rpm,
        max_engine_speed,
        max_torque,
        max_torque_at_rpm,
        max_power_per_liter,
        power_to_weight,
        image
    )
VALUES
    (
        'Turbo Coupe',
        182900,
        572,
        199,
        2.7,
        'Aside from the explosive acceleration, the 911 Turbo exhibits insane levels of cornering grip and strong brakes, making it a true performance powerhouse.',
        '51.3 in',
        '74.9 in',
        '178.6 in',
        '3,635 lb',
        '96.5',
        '3,745 cc twin-turbocharged boxer 6',
        '102.0 mm',
        '76.4 mm',
        '2,981 cc',
        '572 hp',
        '6,500 rpm',
        '7,500 rpm',
        '331 lb-ft',
        '1,950 - 5,000 rpm',
        '126.3 hp/l',
        '8.8 lb/hp',
        'turbo.png'
    ),
    (
        'Turbo S Coupe',
        216100,
        640,
        199,
        2.6,
        'Not the widowmaker 911 Turbo of old. Meticulous engineering has made the even more powerful Turbo S accessible to every driver, regardless of skill.',
        '51.3 in',
        '74.9 in',
        '178.6 in',
        '3,790 lb',
        '96.5',
        '3,745 cc twin-turbocharged boxer 6',
        '102.0 mm',
        '76.4 mm',
        '2,981 cc',
        '572 hp',
        '6,500 rpm',
        '7,500 rpm',
        '331 lb-ft',
        '1,950 - 5,000 rpm',
        '126.3 hp/l',
        '8.8 lb/hp',
        'turbo-s.png'
    ),
    (
        'Turbo Cabriolet',
        195700,
        572,
        205,
        2.8,
        'The drop-top Porsche 911 Turbo has million-mile headroom and performance to blow away supercar A-listers',
        '51.3 in',
        '74.9 in',
        '178.6 in',
        '3,636 lb',
        '96.5',
        '3,745 cc twin-turbocharged boxer 6',
        '102.0 mm',
        '76.4 mm',
        '3,745 cc',
        '640 hp',
        '6,750 rpm',
        '7,200 rpm',
        '590 lb-ft',
        '2,500- 4,000 rpm',
        '168.4 hp/l',
        '5.7 lb/hp',
        'turbo-cab.png'
    ),
    (
        'Turbo S Cabriolet',
        228900,
        640,
        205,
        2.7,
        'An open-air daily driver that hits 60 mph in just 2.7 seconds.',
        '51.3 in',
        '74.9 in',
        '178.6 in',
        '3,790 lb',
        '96.5',
        '3,745 cc twin-turbocharged boxer 6',
        '102.0 mm',
        '76.4 mm',
        '3,745 cc',
        '640 hp',
        '6,750 rpm',
        '7,200 rpm',
        '590 lb-ft',
        '2,500- 4,000 rpm',
        '168.4 hp/l',
        '5.9 lb/hp',
        'turbo-s-cab.png'
    );

INSERT INTO
    gt3_models(
        model_name,
        price,
        power_in_hp,
        top_speed_mph,
        zero_to_sixty,
        description,
        height,
        width,
        length,
        curb_weight,
        wheelbase,
        motor,
        bore,
        stroke,
        displacement,
        max_power,
        max_power_at_rpm,
        max_engine_speed,
        max_torque,
        max_torque_at_rpm,
        max_power_per_liter,
        power_to_weight,
        image
    )
VALUES
    (
        '911 GT3',
        169700,
        502,
        197,
        3.2,
        'The only 992-generation 911 that does not have a turbocharger, the GT3 brings 502 horses of naturally-aspirated joy. The 911 GT3 is not a daily driver, but you will want to drive it every day.',
        '50.4',
        '72.9 in',
        '180.0 in',
        '3,126 lb',
        '96.7 in',
        '4.0-liter naturally aspirated boxer 6',
        '102.0 mm',
        '81.5 mm',
        '3,996 cc',
        '502 hp',
        '8,400 rpm',
        '9,000 rpm',
        '346 lb-ft',
        '6,100 rpm',
        '125.6 hp/l',
        '6.3 lb/hp',
        'gt3.png'
    ),
    (
        '911 GT3 with Touring Package',
        169700,
        502,
        199,
        3.7,
        'The 911 GT3 with Touring Package also sets the performance tone - but with focus primarily on the road. Thanks to ingenious areodynamics, cornering remains impressive, even without a large rear wing.',
        '50.4',
        '72.9 in',
        '180.0 in',
        '3,126 lb',
        '96.7 in',
        '4.0-liter naturally aspirated boxer 6',
        '102.0 mm',
        '81.5 mm',
        '3,996 cc',
        '502 hp',
        '8,400 rpm',
        '9,000 rpm',
        '346 lb-ft',
        '6,100 rpm',
        '125.6 hp/l',
        '6.3 lb/hp',
        'gt3-touring.png'
    );

INSERT INTO
    porsches_4_sale(
        model_name,
        year,
        body_style,
        price,
        generation,
        color,
        transmission,
        description,
        main_image,
        second_image,
        third_image,
        fourth_image,
        fifth_image
    )
VALUES
    (
        'Carrera Cabriolet',
        1994,
        'Cabriolet',
        54900,
        993,
        'Black',
        'manual',
        '1995 Carrera cabriolet with 3.6 liter flat-six paired with the very diserable manual transmission. Only 47,000 miles. Aftermaker exhaust, replacement black soft top, 18" Turbo-style wheels, cruise control and air conditioning. Owners manual, service record, tool kit and a clean Carfax and title present. Unique chance to own an air-cooled 911.',
        'black_993_cab_primary.webp',
        'black_993_cab_2.webp',
        'black_993_cab_3.webp',
        'black_993_cab_4.webp',
        null
    ),
    (
        'Carrera Coupe',
        1995,
        'Coupe',
        98000,
        993,
        'Speed Yellow',
        'manual',
        'Unique opporunity to own a 993 carerra coupe. This striking yellow on black carrera has been with the seller since 2003. This 993 Carrera coupe has 70,000 miles and comes with a car cover, spare set of 18-inch Turbo-style wheels, a clean Carfax and clean title.',
        'yellow_993_coup_primary.webp',
        'yellow_993_coupe_2.webp',
        'yellow_993_coupe_3.webp',
        'yellow_993_coupe_4.webp',
        'yellow_993_coupe_5.webp'
    ),
    (
        'Turbo Coupe',
        2004,
        'Coupe',
        93000,
        996,
        'Black',
        'manual',
        'Probably the best budget 911 Turbo on the market!! Black on black Turbo coupe with only 24,000 miles. Sunroof, xenon headlights, spoiler and stainless steel exhausts. Interior features yellow seatbelts and stitching, metal door sills and a leather interior package as well as modern luxuries such as heated seats, cruise control, navigation system and a Bose sound system. Some light paintwork done on rear bumper but clean Carfax with no accidents or damage.',
        'black_996_coupe_primary.webp',
        'black_996_coupe_2.webp',
        'black_996_coupe_3.webp',
        'black_996_coupe_4.webp',
        'black_996_coupe_5.webp'
    ),
    (
        '911 Speedster',
        2011,
        'Speedster',
        333333,
        997,
        'Special',
        'PDK automatic',
        'A truly one-of-a-kind 911, this 2011 Speedster is one of only 356 Speedsters produced. Special ordered in a Cream over black with orrange trimmed cabin to match the San Francisco Giants colors, this speedster features a PDK automatic transmission, 19-inch Sport wheels, ceramic brakes, heated seats, climate control and the diserable Sport Chrono Package. Only 1,300 miles with orignal window sticket, manufacturer literature, clean carfax and title.',
        '997_speedster_primary.webp',
        '997_speedster_2.webp',
        '997_speedster_3.webp',
        '997_speedster_4.webp',
        '997_speedster_5.webp'
    ),
    (
        'Turbo S Coupe',
        2021,
        'Coupe',
        265000,
        992,
        'Olive',
        'PDK automatic',
        'Finished in a paint-to-sample Olive Green over two tone interior. This Turbo S is like new, with on 230 miles. Features include staggered-diameter Turbo S Exclusive Design wheels finished in black, ceramic-composite brakes with high-gloss black calipers, a front-axle lift system, Lane Change Assist, a glass sunroof, a sport exhaust system, and ventilated front seats. One owner with no accidents or damage.',
        '992_turbo_primary.webp',
        '992_turbo_2.webp',
        '992_turbo_3.webp',
        '992_turbo_4.webp',
        '992_turbo_5.webp'
    ),
    (
        'GTS Cabriolet',
        2021,
        'Cabriolet',
        262000,
        992,
        'White',
        'PDK automatic',
        'Carrera GTS Cabriolet finished in White Matallic over Slate Grey interior.  Features include a black soft top, a lowered sport suspension with PASM, staggered-diameter center-lock wheels, Porsche InnoDrive, ceramic-composite brakes, a front-axle lift system, a surround-view camera system, a sport exhaust system, Power Steering Plus, heated and ventilated front seats, a heated steering wheel, and a Burmester surround-sound system. Clean title and accident-free Carfax report.',
        '992_gts_cab_primary.webp',
        '992_gts_cab_2.webp',
        '992_gts_cab_3.webp',
        '992_gts_cab_4.webp',
        '992_gts_cab_5.webp'
    ),
    (
        'Targa 6-Speed Coupe',
        1997,
        'Coupe',
        76000,
        993,
        'Black',
        'Manual',
        'Well-kept targa from the desirable 993 generation. This example is finished in black and equipped with headlight washers, fog lights, power-adjustable mirrors, a speed-activated rear spoiler, a rear window wiper, and dual exhaust outlets. No accidents or other damage present.',
        '1997_porsche_911-targa_primary.jpg.webp',
        '1997_porsche_911-targa_1.jpg.webp',
        '1997_porsche_911-targa_2.jpg.webp',
        '1997_porsche_911-targa_3.jpg.webp',
        '1997_porsche_911-targa_4.jpg.webp'
    ),
    (
        '911 Turbo Coupe',
        1996,
        'Coupe',
        290000,
        993,
        'Silver',
        'Manual',
        'Once in a lifetime opportunity to own a near new example of the last air-cooled 911 Turbos. This 993 Turbo has only 9,000 miles.  The car is finished in Polar Silver Metallic over black leather and features 18″ Turbo Twist wheels, fog lights, a fixed rear spoiler, a power sunroof, heated power-adjustable front seats, and a Porsche Classic Communication Management stereo.',
        '993-turbo-silver-primary.jpg.webp',
        '993-turbo-silver-2.jpg.webp',
        '993-turbo-silver-3.jpg.webp',
        '993-turbo-silver-4.jpg.webp',
        '993-turbo-silver-5.jpg.webp'
    ),
    (
        '911 Turbo Coupe',
        1996,
        'Coupe',
        266000,
        993,
        'Guards Red',
        'Manual',
        'Low-mileage 993 Turbo Coupe in Guards Red. Equipment includes 18″ Turbo Twist wheels, an electric sunroof, power-adjustable seats, cruise control, self-dimming mirrors, rain-sensing wipers, and an AM/FM/cassette stereo with a six-disc remote CD changer.',
        '993_red_primary.jpg.webp',
        '993_red_1.jpg.webp',
        '993_red_2.jpg.webp',
        '993_red_3.jpg.webp',
        '993_red_4.jpg.webp'
    ),
    (
        '911 Turbo 3.6 Coupe',
        1994,
        'Coupe',
        196000,
        964,
        'White',
        'Manual',
        '964 Turbo in Grand Prix White over grey. Features include polished 18″ Speedline Cup wheels, a stainless steel exhaust system, a power-operated sunroof, power-adjustable front seats, air conditioning, and a Porsche CR-1 cassette stereo with a trunk-mounted CD changer. Only 44k miles and no accidents.',
        '1994_porsche_964-turbo-primary.jpg.webp',
        '1994_porsche_964-turbo-1.jpg.webp',
        '1994_porsche_964-turbo-2.jpg.webp',
        '1994_porsche_964-turbo-3.jpg.webp',
        '1994_porsche_964-turbo-4.jpg.webp'
    ),
    (
        '911 Turbo S X85 Flachbau Coupe',
        1994,
        'Coupe',
        500000,
        964,
        'Red',
        'Manual',
        'Rare chance to own one of only 39 964 Turbos delvered with the Flachbau option. It is finished in Guards Red over Cashmere and black leather upholstery, and power comes from a turbocharged 3.6-liter flat-six paired with a five-speed manual transaxle. Additional features include front sport seats, air conditioning, a powered sunroof, a cassette stereo, and polished 18″ Speedline Cup wheels. CD changer was removed, but will be provided upon sale along with Certificate of Authenticity, clean Carfax and title.',
        'flachbau_primary.jpg.webp',
        'flachbau_1.jpg.webp',
        'flachbau_2.jpg.webp',
        'flachbau_3.jpg.webp',
        'flachbau_4.jpg.webp'
    ),
    (
        '912 Coupe',
        1966,
        'Coupe',
        67000,
        912,
        'Irish Green (repaint)',
        'Manual',
        'This 912 has been with one family for 44 years. efinished in Irish Green over a brown interior, the car is powered by a 1.6-liter flat-four paired with a five-speed manual transaxle. Equipment includes a heater, a Blaupunkt radio, front seat belts, dual carburetors, and 15″ steel wheels.',
        '912_primary.jpg.webp',
        '912_1.jpg.webp',
        '912_2.jpg.webp',
        '912_3.jpg.webp',
        '912_4.jpg.webp'
    );

INSERT INTO
    p_2_p_porsches (
        seller_name,
        seller_number,
        seller_email,
        location_city,
        location_state,
        model_name,
        year,
        mileage,
        body_style,
        asking_price,
        generation,
        color,
        transmission,
        description,
        main_image,
        second_image,
        third_image,
        fourth_image,
        fifth_image
    )
VALUES
    (
        'Theodore Frazier',
        '9119110000',
        'theo@theofrazier.com',
        'New Rochelle',
        'New York',
        'Carrera 4S',
        2005,
        23700,
        'Coupe',
        450000,
        '997',
        'Dark Green',
        'Manual',
        'Near immaculate Carrera 4S in the New York area. Elegant green on tan color combination. Less than 25,000 miles. Available for pickup.',
        null,
        null,
        null,
        null,
        null
    );

INSERT INTO
    porsche_911_gens(
        gen_name,
        year_began,
        year_ended,
        description,
        image
    )
VALUES
    (
        '901',
        '1963',
        '1973',
        'Designed as a larger, more practical version of the 356 with an extra pair of seats, the 901 was the beginning a icon. Early examples used a two-liter engine with only 130 horsepower, but the light weight of just over a ton made up for the lack of power. Engine output increased over the life of the production run, topping out with the 210 2.7 Carrera RS.',
        'generations_first.webp'
    ),
    (
        'G-series',
        '1973',
        '1989',
        'The G generation marked a major change, with a very discernable difference in styling, and technology. This generation ran for almost 20 years. The most unique move was the introduction of turbocharging, represented by the Turbo model which featured 260 horsepower.',
        'generations_G_series.webp'
    ),
    (
        '964',
        '1989',
        '1994',
        'A mild syling change combined with tons of new tech, the 964 appeared to be very similar to the G series, but was 85 percent new. Four wheel drive arrived, along with power steering, ABS and airbags. Considered by many to be the least attractive 911, 964s could be found for relatively cheap on the classic market.',
        'generations_964.webp'
    ),
    (
        '993',
        '1994',
        '1998',
        'Considered by many to have the most beautiful and purest 911 design, the 993 also represented the end of an era, by being the last air-cooled 911. The GT2 version of the 993 began the pratice of ultra high-performance 911s like the GT3.',
        'generations_993.webp'
    ),
    (
        '996',
        '1998',
        '2004',
        'Undoubtably the most controversial 911, the 996 was derided by enthusiasts for "losing character" for the styling change to the headlights and use of a water-cooled engine, as well as the sharing of body panels with the cheaper Boxter model. However, the 996 is the 911 that saved Porsche, being the first mass-produced model. It marked the exponential growth in 911 variants, and the launch of the first ever GT3 variant.',
        'generations_996.webp'
    ),
    (
        '997',
        '2004',
        '2011',
        'Round headlights made a return with the all-new 997. The 997 was the first 911 generation where all models had more than 300 horsepower, with a GT2 variant with 530 horspower. It was not without criticism though, with claims of "deminished character" revolving around steering feel and ironically, it being too "secure" in turns due to stronger grip.',
        'generations_977.webp'
    ),
    (
        '991',
        '2011',
        '2018',
        'The 991 was considered one the best-looking 911s in decades, especially with the new Targa version. There were criticisms regarding the electric steering and the eventual use of turbo engines in almost all variants in the 991.2 version, but that did not stop solid sales for the 991, which included the sale of the one-millionth 911.',
        'generations_991.webp'
    ),
    (
        '992',
        '2018',
        'Present',
        'The current 992-generaton marked perhaps the most extreme styling chage for the 911, with a long light bar in the back and full track width for all models, not just the four-wheel drive and motorsport models. The turbocharged flat-six engines remain, with incredibly low emmssons, yet more power. The 992 shows that high performance cars can join the fight for climate change. With the 992, there are also talks of the first hybrid 911 becoming a reality.',
        'generations_992.webp'
    );

INSERT INTO
    users(
        first_name,
        last_name,
        age,
        email,
        user_name,
        password,
        image
    )
VALUES
    (
        'Theodore',
        'Frazier',
        34,
        'theo@gmail.com',
        'tfrazier',
        'iloveporsches',
        NULL
    );

INSERT INTO
    users_porsche(
        model_name,
        body_style,
        year,
        generation,
        color,
        transmission,
        image
    )
VALUES
    (
        '911 Carrera 4S',
        'Coupe',
        2004,
        '996',
        'Silver',
        'Automatic',
        NULL
    );